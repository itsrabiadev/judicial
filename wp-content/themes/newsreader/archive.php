...
$defaultImagePath = get_theme_file_uri('assets/images/section-title-bg-news.jpg');
...
<?php 
if(is_post_type_archive('petitions')){
    ?>
<style>
    .cs-entry__post-meta{
        display:none;
...
<div id="primary" class="cs-content-area">
    ...
    <?php
    if (is_post_type_archive('post') || is_category() || is_tax('category') || (isset($_GET['taxonomy']) && $_GET['taxonomy'] === 'category')):
        get_template_part('template-parts/archive/posts-filter-form');
    endif;
    ?>
    ...
    <?php if (have_posts()): ?>
        ...
        <div class="cs-posts-area cs-posts-area-posts">
            <div class="cs-posts-area__outer">
                <div class="cs-posts-area__main cs-archive-<?php echo esc_attr($options['layout']); ?> <?php echo esc_attr($main_classes); ?> <?php echo esc_attr($grid_class); ?>"
                    <?php if ('grid' === $options['layout']) { ?> data-pc="<?php echo esc_attr($columns_dk); ?>"
                        data-lt="<?php echo esc_attr($columns_lt); ?>" data-tb="<?php echo esc_attr($columns_tb); ?>"
                        data-mb="<?php echo esc_attr($columns_mb); ?>" <?php } ?>>
                    <?php
                    $counter = 1;
                    // Start the Loop.
                    while (have_posts()) {
