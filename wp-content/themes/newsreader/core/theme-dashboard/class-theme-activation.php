...
if ( ! class_exists( 'CSCO_Theme_Activation' ) ) {
	...
	class CSCO_Theme_Activation {
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_csco_activation_dismissed_handler', array( $this, 'dismissed_handler' ) );
		}
		...
		public function register_settings( $tabs ) {
			...
			$tabs[] = array(
				'slug'     => 'license',
				'label'    => esc_html__( 'License Information', 'newsreader' ),
				'priority' => 10,
		...
		public function render_settings() {
			...
			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( esc_html__( 'You do not have sufficient rights to view this page.', 'newsreader' ) );
			}
			...
			<div class="cs-activation">
				<?php if ( $status && ! get_option( $this->setting_subscribe() ) && 'dev' !== csco_get_license_data( 'item_id' ) ) { ?>
					<div class="cs-activation-postbox">
						...
						<h2 class="hndle"><span><?php esc_html_e( 'Updates', 'newsreader' ); ?></span></h2>

						<div class="inside">
							<p class="cs-theme-license-msg"><?php esc_html_e( 'We set a special price for all new themes for just a few days. Get notified of all introductory, flash and seasonal sales by signing up to our updates.', 'newsreader' ); ?></p>

							<form method="post" class="cs-theme-license-form">
								<?php wp_nonce_field(); ?>
								...
								<table class="form-table">
									<!-- Email Address -->
									<tr>
										<th scope="row"><?php esc_html_e( 'Email Address', 'newsreader' ); ?></label></th>
										<td>
									...
									<tr>
										<th scope="row"></th>
										<td>
										<div class="cs-theme-license-updates">
											<input class="cs-theme-license-newsletter" id="newsletter" name="newsletter" type="checkbox" value="1">

											<label for="newsletter">
												<p><?php esc_html_e( 'By checking this box you agree to our', 'newsreader' ); ?> <a href="https://codesupply.co/terms-and-conditions/" target="_blank"><?php esc_html_e( 'Terms and Conditions', 'newsreader' ); ?></a> <?php esc_html_e( 'and', 'newsreader' ); ?> <a href="https://codesupply.co/privacy-policy/" target="_blank"><?php esc_html_e( 'Privacy Policy', 'newsreader' ); ?></a>.</p>

												<p class="description"><?php esc_html_e( 'You may opt-out any time by clicking the unsubscribe link in the footer of any email you receice from us, or by contacting us at', 'newsreader' ); ?> <a target="_blank" href="mailto:support@codesupply.co"><?php esc_html_e( 'support@codesupply.co', 'newsreader' ); ?></a>.</p>
											</label>
											</div>
										</td>
									</tr>
									...
									<tr>
										<th scope="row"></th>
										<td>
											<button name="action" value="subscribe" type="submit" class="button button-primary button-large" id="publish"><?php esc_html_e( 'Subscribe', 'newsreader' ); ?></button>
										</td>
									</tr>
								</table>
							</form>
				...
				<div class="cs-activation-postbox">
					<?php if ( ! $status ) { ?>
						<h2 class="hndle">
							<span>
								<?php esc_html_e( 'Activation', 'newsreader' ); ?>
							</span>
						</h2>
					<?php } ?>
					...
					<div class="inside">
						...
						<?php if ( $status ) { ?>
							<form method="post">
								...
								<table class="form-table">
									<tr>
										<th scope="row"><?php esc_html_e( 'Active Websites', 'newsreader' ); ?></label></th>
										<td>
											<?php
											...
											if ( $history ) {
												...
												<p>
													...
													<button class="button" type="submit" name="action" value="check"><?php esc_html_e( 'Check Again', 'newsreader' ); ?></button>
												</p>
												<?php
											} elseif ( 'dev' === csco_get_license_data( 'item_id' ) ) {
												esc_html_e( 'Development Mode', 'newsreader' );
											}
											?>
										</td>
									</tr>
						...
						<form method="post">
							...
							<table class="form-table">
								...
								<tr class="<?php echo esc_attr( $status ? 'hidden' : null ); ?>">
									<th scope="row"><?php esc_html_e( 'Purchase Code', 'newsreader' ); ?></label></th>
									<td>
								...
								<!-- Purchase Date -->
								<tr class="<?php echo esc_attr( ! $status ? 'hidden' : null ); ?>">
									<th scope="row"><?php esc_html_e( 'Purchase Date', 'newsreader' ); ?></label></th>
									<td>
								...
								if ( strtotime( $supported_until ) < strtotime( 'now' ) ) {
									?>
									<tr class="<?php echo esc_attr( ! $status ? 'hidden' : null ); ?>">
										<th scope="row"><?php esc_html_e( 'Supported Until', 'newsreader' ); ?></label></th>
										<td>
											<span class="cs-theme-license-supported_until">
												<?php echo esc_html( date( 'F d, Y', strtotime( $supported_until ) ) ); ?>

												<?php esc_html_e( ' (Expired)', 'newsreader' ); ?>
											</span>
											<p class="description"><?php esc_html_e( 'Please renew your item support for any support requests. See', 'newsreader' ); ?> <a href="https://help.market.envato.com/hc/en-us/articles/207886473-Extending-and-Renewing-Item-Support" target="_blank"><?php esc_html_e( 'this document', 'newsreader' ); ?></a> <?php esc_html_e( 'for more details.', 'newsreader' ); ?></p>
										</td>
									</tr>
								<?php } ?>
								<!-- Submitbox -->
								<tr>
									<th scope="row">
										<?php
										if ( $status ) {
											esc_html_e( 'Deactivation', 'newsreader' );
										}
										?>
									</th>
									<td>
										<?php if ( $status ) { ?>
											<button name="action" value="deactivate" type="submit" class="button button-primary button-large" id="publish"><?php esc_html_e( 'Deactivate License', 'newsreader' ); ?></button>
										<?php } ?>

										<?php if ( ! $status ) { ?>
											<button name="action" value="activate" type="submit" class="button button-primary button-large" id="publish"><?php esc_html_e( 'Activate License', 'newsreader' ); ?></button>
										<?php } ?>
									</td>
								</tr>
							</table>
		...
		public function trigger_license() {
			...
			if ( 'subscribe' === $action ) {
				if ( isset( $_POST['email'] ) && $_POST['email'] ) { // Input var ok; sanitization ok.
					...
					$this->msg = $this->set_message( esc_html__( 'Email address is considered invalid.', 'newsreader' ) );
					return;
				}

				if ( ! isset( $_POST['newsletter'] ) ) { // Input var ok; sanitization ok.
					$this->msg = $this->set_message( esc_html__( 'Please agree to our terms and conditions.', 'newsreader' ) );
					return;
				}
			}
			...
			if ( is_wp_error( $response ) ) {
				$this->msg = $this->set_message( esc_html__( 'No connection to the server, try another time, or contact support.', 'newsreader' ) );
				return;
			}
		...
		public function display_license_notice() {
			...
			if ( strtotime( $supported_until ) < strtotime( 'now' ) ) {
				...
				if ( ( ! get_transient( $transient_name ) && $dismissible ) || ! $dismissible ) {
					?>
					<div class="cs-activation-notice notice notice-warning <?php echo esc_attr( $dismissible ); ?>" data-notice="<?php echo esc_attr( $transient_name ); ?>">
						<p><strong><?php esc_html_e( 'Support expired.', 'newsreader' ); ?></strong>
						<?php
							// Translators: theme name and link activation.
							echo wp_kses( sprintf( __( 'Your support for the %1$s theme has expired. Please %2$s for any support requests.', 'newsreader' ), $this->theme_name, '<a href="https://help.market.envato.com/hc/en-us/articles/207886473-Extending-and-Renewing-Item-Support" target="_blank">' . __( 'renew your support license', 'newsreader' ) . '</a>' ), 'common' );
						?>
						</p>
					</div>
					<?php
			...
			if ( count( $actived ) > $count ) {
				...
				if ( ( ! get_transient( $transient_name ) && $dismissible ) || ! $dismissible ) {
					?>
						<div class="cs-activation-notice notice notice-warning <?php echo esc_attr( $dismissible ); ?>" data-notice="<?php echo esc_attr( $transient_name ); ?>">
							<p><strong><?php esc_html_e( 'Multiple active websites detected.', 'newsreader' ); ?></strong> <?php esc_html_e( 'Looks like you’re using the same theme license on multiple websites. A website theme can only be customized to create one customized website according to the ThemeForest license terms. If you want to create a second website from the same theme, you’ll need to', 'newsreader' ); ?> <a href="<?php esc_url( admin_url( '/themes.php?page=theme-dashboard&tab=license' ) ); ?>"><?php esc_html_e( 'purchase another license', 'newsreader' ); ?></a>.</p>
						</div>
					<?php
				}
			}
		...
		public function enqueue_scripts( $page ) {
			...
			<script>
				jQuery(function($) {
					$( document ).on( 'click', '.cs-activation-notice .notice-dismiss', function () {
						jQuery.post( 'ajax_url', {
							action: 'csco_activation_dismissed_handler',
							notice: $( this ).closest( '.cs-activation-notice' ).data( 'notice' ),
						});
					} );
				});
			</script>
			...
			<div class="notice notice-warning notice-alt">
				<p>
					...
					if ( is_customize_preview() ) {
						...
						echo wp_kses( sprintf( __( 'Please %1$s to unlock theme demos.', 'newsreader' ), '<a class="button-link" href="' . admin_url( '/themes.php?page=theme-dashboard&tab=license' ) . '">' . __( 'activate your license', 'newsreader' ) . '</a>' ), 'common' );
					} else {
						// Translators: link activation.
						echo wp_kses( sprintf( __( 'Please %1$s to unlock demo content.', 'newsreader' ), '<a class="button-link" href="' . admin_url( '/themes.php?page=theme-dashboard&tab=license' ) . '">' . __( 'activate your license', 'newsreader' ) . '</a>' ), 'common' );
					}
					?>
				</p>
			</div>
