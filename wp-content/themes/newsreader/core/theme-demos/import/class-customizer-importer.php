	...
	public static function import_customizer_options( $data ) {
		...
		if ( ! is_array( $data ) || ! isset( $data['mods'] ) ) {
			return new WP_Error(
				...
				esc_html__( 'Error: The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'newsreader' )
			);
		}
	...
	private static function import_customizer_images( $mods ) {
		foreach ( $mods as $key => $val ) {
			if ( self::customizer_is_image_url( $val ) ) {
				...
				// Upload @2x image.
				self::customizer_sideload_image(
