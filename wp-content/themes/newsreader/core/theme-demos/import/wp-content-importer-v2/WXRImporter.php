<?php
class CSCO_WXRImporter extends WP_Importer {
	/**
	...
	protected function get_reader( $file ) {
		...
		if ( ! $status ) {
			return new WP_Error( 'wxr_importer.cannot_parse', __( 'Could not open the file for parsing', 'newsreader' ) );
		}
	 ...
	 *
	 * @return CSCO_WXRImportInfo|WP_Error
	 */
			...
			switch ( $reader->name ) {
				case 'wp:wxr_version':
					...
					if ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) ) {
						...
						$this->logger->warning( sprintf( __( 'This WXR file (version %1$s) is newer than the importer (version %2$s) and may not be supported. Please consider updating.', 'newsreader' ), $this->version, self::MAX_WXR_VERSION ) );
					}
	...
	protected function import_start( $file ) {
		if ( ! is_file( $file ) ) {
			return new WP_Error( 'wxr_importer.file_missing', __( 'The file does not exist, please try again.', 'newsreader' ) );
		}
	...
	public function set_user_mapping( $mapping ) {
		foreach ( $mapping as $map ) {
			if ( empty( $map['old_slug'] ) || empty( $map['old_id'] ) || empty( $map['new_id'] ) ) {
				$this->logger->warning( __( 'Invalid author mapping', 'newsreader' ) );
				...
				case 'wp:status':
					...
					if ( 'auto-draft' === $data['post_status'] ) {
						...
						return new WP_Error(
							...
							esc_html__( 'Cannot import auto-draft posts', 'newsreader' ),
							$data
						);
					}
					break;
	 ...
	 * @since 1.0.0
		...
		if ( ! $post_type_object ) {
			...
			$this->logger->warning( sprintf( __( 'Failed to import "%1$s": Invalid post type %2$s', 'newsreader' ), $data['post_title'], $data['post_type'] ) );
		...
		if ( $post_exists ) {
			/* translators: 1:singular_name, 2:post_title */
			$this->logger->info( sprintf( __( '%1$s "%2$s" already exists.', 'newsreader' ), $post_type_object->labels->singular_name, $data['post_title'] ) );

			/**
			 * The wxr_importer.db.post hook.
			 *
			 * @since 1.0.0
			 */
		...
		/**
		 * The wp_import_post_data_processed hook.
		 *
		 * @since 1.0.0
		 */
		...
		if ( 'attachment' === $postdata['post_type'] ) {
			if ( ! $this->options['fetch_attachments'] ) {
				...
				$this->logger->notice( sprintf( __( 'Skipping attachment "%s", fetching attachments disabled', 'newsreader' ), $data['post_title'] ) );
			...
			/**
			 * The wp_import_insert_post hook.
			 *
			 * @since 1.0.0
			 */
		...
		if ( is_wp_error( $post_id ) ) {
			...
			$this->logger->error( sprintf( __( 'Failed to import "%1$s" (%2$s)', 'newsreader' ), $data['post_title'], $post_type_object->labels->singular_name ) );
		...
		/* translators: 1:post_title, 2:singular_name */
		$this->logger->info( sprintf( __( 'Imported "%1$s" (%2$s)', 'newsreader' ), $data['post_title'], $post_type_object->labels->singular_name ) );

		/* translators: 1:original_id, 2:post_id */
		$this->logger->debug( sprintf( __( 'Post %1$d remapped to %2$d', 'newsreader' ), $original_id, $post_id ) );
					...
					if ( 'post_format' === $taxonomy ) {
						...
						if ( empty( $term_id ) ) {
							...
							if ( ! is_wp_error( $t ) ) {
								...
								$this->logger->warning( sprintf( esc_html__( 'Failed to import term: %1$s - %2$s', 'newsreader' ), esc_html( $taxonomy ), esc_html( $term['name'] ) ) );
								continue;
							}
						}
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		// If the URL is absolute, but does not contain address, then upload it assuming.
		...
		if ( ! $info ) {
			return new WP_Error( 'attachment_processing_error', esc_html__( 'Invalid file type', 'newsreader' ) );
		}
	...
	protected function process_author( $data, $meta ) {
		...
		if ( is_wp_error( $user_id ) ) {
			...
			$this->logger->error( sprintf( __( 'Failed to import user "%s"', 'newsreader' ), $userdata['user_login'] ) );
		...
		/* translators: 1:user_login */
		$this->logger->info( sprintf( __( 'Imported user "%s"', 'newsreader' ), $userdata['user_login'] ) );

		/* translators: 1:original_id, 2:user_id */
		$this->logger->debug( sprintf( __( 'User %1$d remapped to %2$d', 'newsreader' ), $original_id, $user_id ) );
	...
	protected function process_term( $data, $meta ) {
		...
		if ( is_wp_error( $result ) ) {
			...
			$this->logger->warning( sprintf( __( 'Failed to import %1$s %2$s', 'newsreader' ), $data['taxonomy'], $data['name'] ) );
		...
		$this->logger->info( sprintf( __( 'Imported "%1$s" (%2$s)', 'newsreader' ), $data['name'], $data['taxonomy'] ) );

		/* translators: 1:original_id, 2:term_id */
		$this->logger->debug(sprintf( __( 'Term %1$d remapped to %2$d', 'newsreader' ), $original_id, $term_id ) );
		...
		foreach ( $meta as $meta_item ) {
			...
			if ( $key ) {
				...
				if ( is_wp_error( $result ) ) {
					...
					$this->logger->warning( sprintf( __( 'Failed to add metakey: %1$s, metavalue: %2$s to term_id: %3$d', 'newsreader' ), $key, $value, $term_id ) );
					...
					$this->logger->debug( sprintf( __( 'Meta for term_id %1$d : %2$s => %3$s ; successfully added!', 'newsreader' ), $term_id, $key, $value ) );
				}
	...
	protected function fetch_remote_file( $url, $post ) {
		...
		if ( 200 !== $code ) {
			...
			return new WP_Error( 'import_file_error', sprintf( __( 'Remote server returned %1$d %2$s for %3$s', 'newsreader' ), $code, get_status_header_desc( $code ), $url ) );
		}
		...
		// unlink( $upload['file'] );
		// return new WP_Error( 'import_file_error', __( 'Remote file is incorrect size', 'newsreader' ) );
		// }

		if ( 0 === $filesize ) {
			unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded', 'newsreader' ) );
		}
		...
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			...
			$message = sprintf( __( 'Remote file is too large, limit is %s', 'newsreader' ), size_format( $max_size ) );
	...
	protected function post_process_posts( $todo ) {
		foreach ( $todo as $post_id => $_ ) {
			...
			$this->logger->debug( sprintf( __( 'Running post-processing for post %d', 'newsreader' ), $post_id ) );
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['post'][ $parent_id ] ) ) {
					...
					$this->logger->warning( sprintf( __( 'Could not find the post parent for "%1$s" (post #%2$d)', 'newsreader' ), get_the_title( $post_id ), $post_id ) );
					/* translators: 1:post_id, 2:parent_id */
					$this->logger->debug( sprintf( __( 'Post %1$d was imported with parent %2$d, but could not be found', 'newsreader' ), $post_id, $parent_id ) );
				}
			}
			...
			if ( ! empty( $author_slug ) ) {
				...
				if ( isset( $this->mapping['user_slug'][ $author_slug ] ) ) {
					...
					/* translators: 1:title, 2:post_id */
					$this->logger->warning( sprintf( __( 'Could not find the author for "%1$s" (post #%2$d)', 'newsreader' ), get_the_title( $post_id ), $post_id ) );
					/* translators: 1:post_id, 2:author_slug */
					$this->logger->debug( sprintf( __( 'Post %1$d was imported with author "%2$s", but could not be found', 'newsreader' ), $post_id, $author_slug ) );
				}
			}
			...
			if ( empty( $data ) ) {
				/* translators: 1:post_id */
				$this->logger->debug( sprintf( __( 'Post %d was marked for post-processing, but none was required.', 'newsreader' ), $post_id ) );
			...
			if ( is_wp_error( $result ) ) {
				/* translators: 1:post_id */
				$this->logger->warning( sprintf( __( 'Could not update "%1$s" (post #%2$d) with mapped data', 'newsreader' ), get_the_title( $post_id ), $post_id ) );
	...
	protected function post_process_menu_item( $post_id ) {
		...
		if ( ! empty( $menu_object ) ) {
			...
			/* translators: 1:title, 2:post_id */
			$this->logger->warning( sprintf( __( 'Could not find the menu object for "%1$s" (post #%2$d)', 'newsreader' ), get_the_title( $post_id ), $post_id ) );
			/* translators: 1:post_id, 2:menu_object_id */
			$this->logger->debug( sprintf( __( 'Post %1$d was imported with object "%2$d" of type "%3$s", but could not be found', 'newsreader' ), $post_id, $menu_object_id, $menu_item_type ) );
		}
		...
		foreach ( $todo as $comment_id => $_ ) {
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['comment'][ $parent_id ] ) ) {
					...
					$this->logger->warning( sprintf( __( 'Could not find the comment parent for comment #%d', 'newsreader' ), $comment_id ) );

					/* translators: 1:comment_id, 2:parent_id */
					$this->logger->debug( sprintf( __( 'Comment %1$d was imported with parent %2$d, but could not be found', 'newsreader' ), $comment_id, $parent_id ) );
				}
			}
			...
			if ( ! empty( $author_id ) ) {
				...
				if ( isset( $this->mapping['user'][ $author_id ] ) ) {
					...
					/* translators: 1:comment_id */
					$this->logger->warning( sprintf( __( 'Could not find the author for comment #%d', 'newsreader' ), $comment_id ) );

					/* translators: 1:comment_id, 2:author_id */
					$this->logger->debug( sprintf( __( 'Comment %1$d was imported with author %2$d, but could not be found', 'newsreader' ), $comment_id, $author_id ) );
				}
			}
			...
			if ( empty( $result ) ) {
				/* translators: 1:comment_id */
				$this->logger->warning( sprintf( __( 'Could not update comment #%d with mapped data', 'newsreader' ), $comment_id ) );
	...
	protected function post_process_terms( $terms_to_be_remapped ) {
		...
		foreach ( $terms_to_be_remapped as $termid => $term_taxonomy ) {
			...
			if ( empty( $termid ) || ! is_numeric( $termid ) ) {
				...
				$this->logger->warning( sprintf( __( 'Faulty term_id provided in terms-to-be-remapped array %s', 'newsreader' ), $termid ) );
			...
			if ( empty( $term_taxonomy ) ) {
				...
				$this->logger->warning( sprintf( __( 'No taxonomy provided in terms-to-be-remapped array for term #%d', 'newsreader' ), $term_id ) );
			...
			if ( empty( $parent_slug ) ) {
				/* translators: 1:term_id */
				$this->logger->warning( sprintf( __( 'No parent_slug identified in remapping-array for term: %d', 'newsreader' ), $term_id ) );
			...
			if ( ! isset( $this->mapping['term_slug'][ $parent_slug ] ) || ! is_numeric( $this->mapping['term_slug'][ $parent_slug ] ) ) {
				...
				$this->logger->warning( sprintf( __( 'The term(%1$d)"s parent_slug (%2$s) is not found in the remapping-array.', 'newsreader' ), $term_id, $parent_slug ) );
			...
			if ( empty( $termattributes ) ) {
				/* translators: 1:term_id */
				$this->logger->warning( sprintf( __( 'No data returned by get_term_by for term_id #%d', 'newsreader' ), $term_id ) );
			...
			if ( is_wp_error( $result ) ) {
				...
				$this->logger->warning( sprintf( __( 'Could not update "%1$s" (term #%2$d) with mapped data', 'newsreader' ), $termattributes['name'], $term_id ) );
			...
			$this->logger->debug( sprintf( __( 'Term %1$d was successfully updated with parent %2$d', 'newsreader' ), $term_id, $mapped_parent ) );
		}
	}
	...
	public function remap_featured_images() {
		...
		$this->logger->info( esc_html__( 'Starting remapping of featured images', 'newsreader' ) );
		...
		foreach ( $this->featured_images as $post_id => $value ) {
			if ( isset( $this->mapping['post'][ $value ] ) ) {
				...
				if ( $new_id !== $value ) {
					...
					$this->logger->info( sprintf( esc_html__( 'Remapping featured image ID %1$d to new ID %2$d for post ID %3$d', 'newsreader' ), $value, $new_id, $post_id ) );
	...
	protected function max_attachment_size() {
		/**
		 * The import_attachment_size_limit hook.
		 *
		 * @since 1.0.0
		 */
	...
	protected function prefill_existing_posts() {
		...
		$posts = $wpdb->get_results( $wpdb->prepare( "SELECT ID, guid FROM {$wpdb->posts}" ) );
	...
	protected function prefill_existing_terms() {
		...
		$terms = $wpdb->get_results( $wpdb->prepare( "SELECT t.term_id, tt.taxonomy, t.slug FROM {$wpdb->terms} AS t JOIN {$wpdb->term_taxonomy} AS tt ON t.term_id = tt.term_id" ) );
