...
class CSCO_Mega_Menu {
	...
	public function __construct() {
		...
		// Set locations.
		$this->locations = apply_filters( 'csco_mega_menu_locations', array( 'primary', 'mobile' ) );
	...
	public function mega_menu_output( $item_output, $item, $depth, $args ) {
		...
		$scheme = csco_color_scheme(
			get_theme_mod( 'color_header_elements_background', '#f8f8f8' ),
			get_theme_mod( 'color_header_elements_background_dark', '#292929' )
		);
		...
		switch ( $menu_type ) {
			case 'mixed':
				if ( isset( $item->mega_menu_children ) && ! empty( $item->mega_menu_children ) ) {
					...
					if ( 'terms' === $layout ) {
						?>
						<div class="sub-menu" <?php echo wp_kses( $scheme, 'csco' ); ?>>
	...
	public function rest_api_callback( $request ) {
		...
		// Number of posts.
		$per_page = isset( $_POST['per_page'] ) ? (int) $_POST['per_page'] : 4;  // Input var ok; sanitization ok.

		// Term ID.
		$term_id = isset( $_POST['term'] ) ? (int) $_POST['term'] : 0;  // Input var ok; sanitization ok.

		// Posts.
		$posts = isset( $_POST['posts'] ) ? (string) $_POST['posts'] : 0;  // Input var ok; sanitization ok.
		...
		if ( 7 === $per_page ) {
			...
			<div class="cs-mm__posts-mixed">
				...
				if ( $query->have_posts() && isset( $query->posts ) ) {
					...
					<div class="cs-mm__posts-mixed-trending">
						<h3 class="cs-mm__posts-mixed-title"><?php esc_html_e( 'Trending Now', 'newsreader' ); ?></h3>
						...
						<div class="cs-mm__posts-mixed-list">
							...
							while ( $query->have_posts() ) {
								$query->the_post();
								?>
								<article <?php post_class( 'mega-menu-item menu-post-item menu-post-item-tile' ); ?>>
									<div class="cs-entry__outer cs-entry__overlay cs-overlay-ratio cs-ratio-landscape" data-scheme="inverse">
										<div class="cs-entry__inner cs-entry__thumbnail">
											<div class="cs-overlay-background">
												<?php if ( has_post_thumbnail() ) { ?>
													<?php the_post_thumbnail( 'csco-medium-uncropped', array( 'loading' => 'lazy' ) ); ?>
												<?php } ?>
											</div>
										</div>

										<div class="cs-entry__inner cs-entry__content cs-overlay-content">
											<?php csco_get_post_meta( array( 'category' ), true, 'mega_menu_post_meta' ); ?>

											<?php the_title( '<h2 class="cs-entry__title">', '</h2>' ); ?>

											<?php csco_get_post_meta( array( 'author', 'date', 'views', 'comments' ), true, 'mega_menu_post_meta' ); ?>
										</div>
				...
				if ( $query->have_posts() && isset( $query->posts ) ) {
					...
					<div class="cs-mm__posts-mixed-popular">
						<h3 class="cs-mm__posts-mixed-title"><?php esc_html_e( 'Most popular', 'newsreader' ); ?></h3>
						...
						<div class="cs-mm__posts-mixed-list">
							...
							while ( $query->have_posts() ) {
								$query->the_post();
								?>
								<article <?php post_class( 'mega-menu-item menu-post-item menu-post-item-horizontal' ); ?>>
									<div class="cs-entry__outer">
										<?php if ( has_post_thumbnail() ) { ?>
											<div class="cs-entry__inner cs-entry__overlay cs-entry__thumbnail cs-overlay-ratio cs-ratio-square">
												<div class="cs-overlay-background">
													<?php the_post_thumbnail( 'csco-small', array( 'loading' => 'lazy' ) ); ?>
												</div>
			...
			if ( $query->have_posts() && isset( $query->posts ) ) {
				// Set options.
				$options = array(
					'image_orientation' => get_theme_mod( 'mega_menu_image_orientation', 'landscape-3-2' ),
					'image_size'        => get_theme_mod( 'mega_menu_image_size', 'csco-medium-uncropped' ),
				);
					...
					<article <?php post_class( 'mega-menu-item menu-post-item menu-post-item-tile' ); ?>>
						<div class="cs-entry__outer cs-entry__overlay cs-overlay-ratio cs-ratio-<?php echo esc_attr( $options['image_orientation'] ); ?>" data-scheme="inverse">
							<div class="cs-entry__inner cs-entry__thumbnail">
								<div class="cs-overlay-background">
									<?php if ( has_post_thumbnail() ) { ?>
										<?php the_post_thumbnail( $options['image_size'], array( 'loading' => 'lazy' ) ); ?>
									<?php } ?>
								</div>
							</div>
