...
class DeployerServiceProvider
{
    ...
    /**
     * Build a POST-shaped array from a Gravity Forms entry (for Deployer field mapping).
     *
     * @param array $entry
     * @param array $form
     * @param array $extra
     * @return array
     */
    public function buildPostRequestFromEntry($entry, $form, $extra = [])
    {
        $postRequest = array_merge(
            [
                'gform_submit' => rgar($form, 'id'),
            ],
            $extra
        );
        ...
        foreach ($form['fields'] as $field) {
            if (!empty($field->inputs) && is_array($field->inputs)) {
                foreach ($field->inputs as $input) {
                    $inputId = str_replace('.', '_', (string) $input['id']);
                    $postRequest['input_' . $inputId] = rgar($entry, (string) $input['id']);
                }
            } else {
                $postRequest['input_' . $field->id] = rgar($entry, (string) $field->id);
            }
        }

        if (!empty($_POST['virtual_fields']) && is_array($_POST['virtual_fields'])) {
            $postRequest['virtual_fields'] = $_POST['virtual_fields'];
        }
    ...
    public function addOrUpdateSubscriber($email, $customDeployerFields = null)
    {
        ...
        if (!($customDeployerFields instanceof Collection)) {
            $customDeployerFields = collect($customDeployerFields);
        }

        $submissionUrl = (string) $customDeployerFields->get('50', '');
        $StringUrl = array_slice(explode('/', urldecode($submissionUrl)), 1);
        //echo "</pre>";
        //print_r($StringUrl);
        //echo "<br>";
        ...
        // Build XML string to match the provided Twig template
        $xmlString = "<xmlrequest>\n";
        $xmlString .= "    <username>" . htmlspecialchars($this->apiUser) . "</username>\n";
        $xmlString .= "    <usertoken>" . htmlspecialchars($this->apiKey) . "</usertoken>\n";
        $xmlString .= "    <requesttype>subscribers</requesttype>\n";
        $xmlString .= "    <requestmethod>AddOrUpdateSubscriber</requestmethod>\n";
        $xmlString .= "    <details>\n";
        $xmlString .= "        <emailaddress>" . htmlspecialchars($email) . "</emailaddress>\n";
        $xmlString .= "        <sms_mobile>" . htmlspecialchars($sms_mobile) . "</sms_mobile>\n";
        $xmlString .= "        <listgroupid>" . htmlspecialchars($listGroupId) . "</listgroupid>\n";
        $xmlString .= "        <listid>" . htmlspecialchars($listId) . "</listid>\n";
        $xmlString .= "        <!-- mailinglist>" . (isset($mailingListId) ? htmlspecialchars($mailingListId) : '') . "</mailinglist -->\n";
        $xmlString .= "        <format>html</format>\n";
        $xmlString .= "        <confirmed>yes</confirmed>\n";
        $xmlString .= "        <customfields>\n";
        foreach ($costumArray as $deployerFieldKey => $deployerFieldValue) {
            $xmlString .= "            <item>\n";
            $xmlString .= "                <fieldid>" . htmlspecialchars($deployerFieldKey) . "</fieldid>\n";
            $xmlString .= "                <value>" . htmlspecialchars($deployerFieldValue) . "</value>\n";
            $xmlString .= "            </item>\n";
        }
        $xmlString .= "        </customfields>\n";
        $xmlString .= "        <opt_in>" . htmlspecialchars($opt_in) . "</opt_in>\n";
        $xmlString .= "    </details>\n";
        $xmlString .= "</xmlrequest>\n";
        //echo "<pre>";
        //print_r($xmlString);
